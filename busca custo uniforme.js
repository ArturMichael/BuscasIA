class Cidade {
  ...
  function buscaCustoUniforme(inicio, objetivo) {
    let fila = [];
    let custoAtual = new Map(); // Armazena o custo atual para cada cidade
    inicio.visitado = true;
    fila.push(inicio);
    custoAtual.set(inicio, 0); // O custo para chegar à cidade inicial é 0
  
    while (fila.length > 0) {
        fila.sort((a, b) => custoAtual.get(a) - custoAtual.get(b)); // Ordena a fila pelo custo atual
        ...
        for (let vizinho of cidadeAtual.vizinhos) {
            let novoCusto = custoAtual.get(cidadeAtual) + vizinho.distancia;
            if (!vizinho.cidade.visitado || novoCusto < custoAtual.get(vizinho.cidade)) {
                vizinho.cidade.visitado = true;
                fila.push(vizinho.cidade);
                custoAtual.set(vizinho.cidade, novoCusto);
            }
        }
    }
    return false;
  ...
  if (buscaCustoUniforme(zerind, bucharest)) {
